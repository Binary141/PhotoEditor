...
void keyboard_cb(unsigned char c, int x, int y) {
  ...
  case 'c':
    ...
    break;
  case '1':
    g_app_data->NewColorTable();
    break;
  
  case 'T':
    g_app_data->setInteractionMode(InteractionMode::IM_COLORTABLE);
    break;
  case 't':
    g_app_data->setInteractionMode(InteractionMode::IM_FRACTAL);
    break;
  ...
  case 'b':
    g_app_data->setFractalMode(FractalMode::M_MANDELBROT);
    g_app_data->createFractal();
    break;

  case 'n':
  g_app_data->setFractalMode(FractalMode::M_JULIA);
    g_app_data->createFractal();
    break;

  case 'F':
    g_app_data->setFractalMode(FractalMode::M_COMPLEX);
    g_app_data->createFractal();
    break;
